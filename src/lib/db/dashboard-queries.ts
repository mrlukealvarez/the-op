...
export async function getCafeEvents(limit?: number): Promise<CafeEvent[] | null> {
  const supabase = createAdminClient()
  if (!supabase) return null
...
export async function getEventsByType(): Promise<Array<{ type: string; count: number; revenue: number }> | null> {
  const supabase = createAdminClient()
  if (!supabase) return null
  ...
  return Object.entries(grouped).map(([type, v]) => ({ type, count: v.count, revenue: v.revenue }))
}
